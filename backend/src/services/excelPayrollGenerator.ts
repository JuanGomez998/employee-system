...
import { PayrollCalculation } from "./payrollService";
...
export async function generatePayrollExcel(
  ...
  companyName: string = "Employee System"
): Promise<Buffer> {
...
function createSummarySheet(
  ...
  payrolls.forEach((payroll) => {
    ...
    cells[4].value = (
      ...
      payroll.combinedExtraHolidayValue
    ).toFixed(2);
...
function createAnalysisSheet(
  ...
  summaryData.forEach((item) => {
    ...
    sheet.getCell(row, 3).value = parseFloat(item[2] as string);
