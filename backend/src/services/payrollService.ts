...
export interface HourBreakdown {
  normalHours: number;
  extraDiurnHours: number;
  extraNocturnHours: number;
  nocturnSurchargeHours: number;
  sundayHours: number;
  holidayHours: number;
  combinedExtraSundayHours: number;
  combinedExtraHolidayHours: number;
}
...
export interface PayrollCalculation {
  ...
  // Valores
  normalHoursValue: number;
  extraDiurnValue: number;
  extraNocturnValue: number;
  nocturnSurchargeValue: number;
  sundayValue: number;
  holidayValue: number;
  combinedExtraSundayValue: number;
  combinedExtraHolidayValue: number;
  
  subtotalHours: number;
  
  // Prestaciones sociales
  prima: number; // Prima legal: salario / 12
  cesantia: number; // Cesantía: salario / 12
  cesantiaInterest: number; // Interés cesantía: 12% anual
  vacacionesProporcionales: number; // Vacaciones: salario / 30
  auxilioTransporte: number; // Auxilio transporte
 ...
 */
function calculateHoursBetween(checkIn: Date, checkOut: Date): number {
 ...
 */
function isNocturnHour(hour: number): boolean {
 ...
 */
export function breakdownDayHours(dayRecord: DayRecord): HourBreakdown {
...
export function calculateBenefits(
  ...
  return {
    ...
    auxilioTransporte,
    totalBenefits:
      prima + cesantia + cesantiaInterest + vacacionesProporcionales + auxilioTransporte,
  };
}
...
export function calculatePayroll(
  ...
  // Totalizar horas
  let totalBreakdown: HourBreakdown = {
    normalHours: 0,
    extraDiurnHours: 0,
    extraNocturnHours: 0,
