...
import { calculatePayroll, DayRecord } from "../services/payrollService";
import { generatePayrollExcel } from "../services/excelPayrollGenerator";
 ...
 * }
 */
export async function generatePayrollReport(req: Request, res: Response) {
  try {
    ...
    const excelBuffer = await generatePayrollExcel(mockPayrolls, "Employee System");
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=payroll_${new Date().toISOString().split("T")[0]}.xlsx`
    );
 ...
 */
function generateMockPayrolls(startDate: Date, endDate: Date, employeeCedulas?: string[]) {
  const employees = [
  ...
  const normalizedCedulas = employeeCedulas?.map((item: string) => item.trim().toUpperCase());
  ...
  return filteredEmployees.map((emp) => {
    ...
    return calculatePayroll(emp.cedula, emp.name, emp.salary, dayRecords, startDate, endDate);
  });
}
